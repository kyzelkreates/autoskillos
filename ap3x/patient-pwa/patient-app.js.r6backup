...
function queueDmSyncItem(payload) {
  try {
    const KEY = 'ap3x_dm_pwa_sync_queue';
    const queue = JSON.parse(localStorage.getItem(KEY) || '[]');
    queue.push({
      id:         'sync_pwa_' + Date.now(),
    ...
    // Non-fatal — sync queue failure must never block PWA
  }
}
...
function completeLessonFromDetail(id, xpVal) {
  ...
  // ── Run 2: Also write to structured DM progress record ─────────────────
...
function renderPathway() {
  ...
  if (!emp) {
    el.innerHTML =
      ...
      '<div style="padding:32px 16px;text-align:center;color:var(--muted)">' +
        ...
        '<div style="font-size:13px;line-height:1.6">Demo employee profiles appear when Demo Mode is active. Live employee profiles will appear after backend/live setup is configured.</div>' +
      '</div>' +
      '<div class="safety-notice" style="margin:0 0 16px">Your progress is saved locally first. Dashboard sync will be connected in a later run or through live backend setup.</div>';
    return;
  }
  ...
  el.innerHTML =
    ...
    '</div>' +
    empPathHtml +
    '<div class="safety-notice" style="margin-top:4px">Your progress is saved locally first. Dashboard sync will be connected in a later run or through live backend setup.</div>';
}
...
function handleSafetyAck(ackId) {
  ...
  showToast('&#x2713; Safety acknowledgement saved locally');
  renderSafetyChecks();
  updateHeader();
}
...
function renderProgress() {
  ...
  el.innerHTML =
    ...
    '<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:14px;margin-bottom:14px">' +
      '<div style="font-size:12px;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">&#x1F4BE; Local Storage Status</div>' +
      '<div style="font-size:13px;line-height:1.8">' +
        '&#x2713; Progress saved locally &mdash; local-first mode active<br>' +
        (syncQ > 0 ? '&#x1F504; ' + syncQ + ' updates queued for future dashboard sync<br>' : '&#x2713; Sync queue empty<br>') +
        '<span style="color:var(--muted);font-size:12px">Backend: ' + status.backendMode + ' &mdash; live sync connects in a later run</span>' +
      '</div>' +
    '</div>' +
    '<button onclick="submitCheckin()" aria-label="Log training check-in" style="width:100%;padding:13px;background:var(--gold);color:#000;border:none;border-radius:var(--r);font-size:14px;font-weight:700;cursor:pointer;margin-bottom:14px">&#x1F4CB; Log Training Check-In (+10 XP)</button>' +
    '<div class="safety-notice">' +
